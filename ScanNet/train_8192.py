...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            loss = MODEL.get_loss1(pred, labels_pl, smpws_pl)
...
def get_batch_3(idx):
    batch_data, batch_label, batch_smpw = TEST_DATASET_WHOLE_SCENE[idx]
    current_data_item = []
    current_label_item = []
    current_smpw_item = []
    current_choice = []
    for i in range(batch_data.shape[0]):
        ...
        label_item = batch_label[i,:]
        smpw_item = batch_smpw[i,:]
    ...
    return current_data_item,current_label_item,current_smpw_item
...
def eval_whole_scene_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        if not is_continue_batch:
            batch_data, batch_label, batch_smpw = get_batch_3(batch_idx)
            ...
            batch_data_tmp, batch_label_tmp, batch_smpw_tmp = get_batch_3(batch_idx)
