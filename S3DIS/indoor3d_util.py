...
def room2blocks(data, label, num_point, block_size=2.0, stride=1.5,
    ...
    for idx in range(len(xbeg_list)): 
       ...
       if np.sum(cond) < 100: # discard block if there are less than 100 pts.
           continue
       xcond = (data[:,0]<=xbeg+block_size/2) & (data[:,0]>=xbeg)
       ycond = (data[:,1]<=ybeg+block_size/2) & (data[:,1]>=ybeg)
       block_data = data[cond, :]
       block_label = label[cond]
       block_data_sampled_1, block_label_sampled_1 = sample_data_label(block_data, block_label, int(num_point/4))
       
       xcond = (data[:,0]<=xbeg+block_size) & (data[:,0]>=xbeg+block_size/2)
       ycond = (data[:,1]<=ybeg+block_size/2) & (data[:,1]>=ybeg)
       block_data = data[cond, :]
       block_label = label[cond]
       block_data_sampled_2, block_label_sampled_2 = sample_data_label(block_data, block_label, int(num_point/4))

       xcond = (data[:,0]<=xbeg+block_size/2) & (data[:,0]>=xbeg)
       ycond = (data[:,1]<=ybeg+block_size) & (data[:,1]>=ybeg+block_size/2)
       block_data = data[cond, :]
       block_label = label[cond]
       block_data_sampled_3, block_label_sampled_3 = sample_data_label(block_data, block_label, int(num_point/4))

       xcond = (data[:,0]<=xbeg+block_size) & (data[:,0]>=xbeg+block_size/2)
       ycond = (data[:,1]<=ybeg+block_size) & (data[:,1]>=ybeg+block_size/2)
       block_data = data[cond, :]
       block_label = label[cond]
       block_data_sampled_4, block_label_sampled_4 = sample_data_label(block_data, block_label, int(num_point/4))
       # randomly subsample data
       block_data_sampled = np.concatenate([block_data_sampled_1, block_data_sampled_2], axis=0)
       block_data_sampled = np.concatenate([block_data_sampled, block_data_sampled_3], axis=0)
       block_data_sampled = np.concatenate([block_data_sampled, block_data_sampled_4], axis=0)
       block_label_sampled = np.concatenate([block_label_sampled_1, block_label_sampled_2], axis=0)
       block_label_sampled = np.concatenate([block_label_sampled, block_label_sampled_3], axis=0)
       block_label_sampled = np.concatenate([block_label_sampled, block_label_sampled_4], axis=0)
...
def room2blocks_wrapper_normalized(data_label_filename, num_point, block_size=2.0, stride=1.5,
                                   random_sample=False, sample_num=None, sample_aug=1):
    if data_label_filename[-3:] == 'txt':
        data_label = np.loadtxt(data_label_filename)
    elif data_label_filename[-3:] == 'npy':
